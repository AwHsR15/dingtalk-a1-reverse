...
RIFF 风格分块结构,但**有两处不标准**,踩过才知道。完整头部图(v1.7,
偏移十进制,已在真机文件上逐字节对齐):
...
偏移  内容
  ...
  8   "DTYJ"
 12   "ver "            ← 特例①:没有长度字段
 16   "v1.7"                  tag 之后直接就是 4 字节版本串
 20   "fmt "
 24   u32   块长 = 24
 28   块体开始
 36     u32  采样率(实测 16000)
 48     u32  时长(毫秒,和界面显示吻合)
 52   "extr"
 56   u32   块长 = 8
 68   "data"
 72   u32   载荷字节数
 76   u32   CRC32          ← 特例②:长度之后还插了 CRC
 80   载荷开始
```

> 这里有**三处不标准**,踩全了才能解对:
>
> **① `ver ` 没有长度字段** —— tag 之后直接就是 4 字节版本串。
> 按标准块(tag+size+body)走,后面每一块都会被推后 4 字节。
>
> **② `fmt ` 的长度字段写的是 0** —— 实际块体固定 24 字节。
> 这一条最阴:通用 RIFF 遍历会在这里读到 size=0,然后把紧随其后的
> 码率字节(`00 7d 00 00`)当成下一个块的 tag,整个解析当场跑飞。
>
> **③ `data` 的长度之后还插了 4 字节 CRC32** 才到载荷。
> 把 CRC 当成载荷首 4 字节,之后**每条记录都错位 4 字节**,TOC 全乱
> —— 看起来会非常像"包体被加密了",极易误判。
>
> 结论:**这个容器长得像 RIFF,但不能用通用 RIFF 解析器读**,
> 必须认死 80 字节固定头。自检很便宜:`ver `@12、`fmt `@20、
> `extr`@52、`data`@68 四个 tag 必须各就各位。

这两处正是客户端 `BabaContainer.kt` 里那套绝对偏移的由来;
它靠这套偏移查出了**一批被截断的录音**(见下)。

#### data 块排布(2026-09 解出)

这一节原先写着"尚未解出"。公开项目
[Shawn-TKD/dingtalk-a1-pc-tools](https://github.com/Shawn-TKD/dingtalk-a1-pc-tools)
的 `tools/dtyj_to_ogg.py` 给出了关键偏移,经与本项目自己的数据交叉验证后确认:

`data` 块是**定长记录数组**,每条记录 `[0:4]` 是前缀、`[4:]` 是一个 Opus 包。
记录长度写在 `fmt` 块里。以下偏移**已在三份真机文件头上逐一验证**
(样本取自客户端 `BabaContainerTest.kt` 的 fixture,那是从真机抓下来的):
...
注意别把 84 和 80 搞混:官方 native SDK 打印的 `Frame size: 80` 指的是
**Opus 包**大小,不是记录大小。

**三条互相独立的校验,全部通过**:

1. `80 + 载荷长 == BABA@4 + 8`(即声明的文件总长)—— 三份样本全中;
2. 载荷长能被 84 **整除** —— 三份样本余数均为 0
   (257984 / 111600 / 1376564 条记录);
3. **记录数 × 20 ms 等于界面显示的时长** —— 样本 1 的 257984 条
   = 5159.7 秒 = 1 小时 25 分 60 秒,正是那条标着「1h 26m 5s」的录音。

第 3 条尤其关键:它把"容器解析"和"用户看得见的时长"这两件独立的事对上了。

> Ogg 封装侧的 granulepos 恒以 48 kHz 计,所以一个 20 ms 帧记 960 —— 
> 这与"源采样率 16 kHz"并不矛盾,是 Ogg Opus 的规定,别把两者搞混。

`tools/dtyj_parse.py` 实现了这套解析,并且**不预设结论**:它把几种
(记录长度, 前缀长度)组合当假设,用 TOC 集中度去判定哪个成立。

#### 但是:PC-tools 的转换器不解密

他们的 `dtyj_to_ogg.py` **没有任何解密逻辑**,直接把包体塞进 Ogg。
这与本项目实测冲突 —— 见第 12 节:`AES flag: 1` 是**默认开着**的,
包体是密文,裸转封装出来的 Ogg 框架合规但 ffmpeg 只能啃出噪声。

所以两种可能:要么他们的设备/固件没开加密,要么他们的输出其实也是坏的
而未被察觉。`dtyj_parse.py` 的 TOC 集中度正好能区分这两种情况:

- 集中度 ≈ 100% → 明文,可直接转封装;
- 集中度 ≈ 1/256 → 密文,**必须**先过官方 `openAudioFile(deviceSecret)`。

客户端因此仍原样存成 `.dtyj`,不改后缀成 `.opus`(会让人以为能直接播)。
